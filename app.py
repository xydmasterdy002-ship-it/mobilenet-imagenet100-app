import streamlit as st
import torch
import torch.nn as nn
from PIL import Image
from torchvision import transforms
import json
import os
import matplotlib.pyplot as plt
from googletrans import Translator

# ======================================================
# ImageNet Normalize
# ======================================================
IMAGENET_MEAN = [0.485, 0.456, 0.406]
IMAGENET_STD = [0.229, 0.224, 0.225]

transform = transforms.Compose([
    transforms.Resize(256),
    transforms.CenterCrop(224),
    transforms.ToTensor(),
    transforms.Normalize(IMAGENET_MEAN, IMAGENET_STD)
])

# ======================================================
# MobileNetV1 定義
# ======================================================
class MobileNetV1(nn.Module):
    def __init__(self, num_classes=100):
        super().__init__()

        def conv_bn(inp, oup, stride):
            return nn.Sequential(
                nn.Conv2d(inp, oup, 3, stride, 1, bias=False),
                nn.BatchNorm2d(oup),
                nn.ReLU6(inplace=True)
            )

        def conv_dw(inp, oup, stride):
            return nn.Sequential(
                nn.Conv2d(inp, inp, 3, stride, 1, groups=inp, bias=False),
                nn.BatchNorm2d(inp),
                nn.ReLU6(inplace=True),

                nn.Conv2d(inp, oup, 1, 1, 0, bias=False),
                nn.BatchNorm2d(oup),
                nn.ReLU6(inplace=True),
            )

        self.model = nn.Sequential(
            conv_bn(3, 32, 2),
            conv_dw(32, 64, 1),
            conv_dw(64, 128, 2),
            conv_dw(128, 128, 1),
            conv_dw(128, 256, 2),
            conv_dw(256, 256, 1),
            conv_dw(256, 512, 2),
            conv_dw(512, 512, 1),
            conv_dw(512, 512, 1),
            conv_dw(512, 512, 1),
            conv_dw(512, 512, 1),
            conv_dw(512, 512, 1),
            conv_dw(512, 1024, 2),
            conv_dw(1024, 1024, 1),
            nn.AdaptiveAvgPool2d(1)
        )

        self.fc = nn.Linear(1024, num_classes)

    def forward(self, x):
        x = self.model(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        return x

# ======================================================
# 載入模型與 Labels
# ======================================================
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
st.write(f"Using device: {device}")

base_path = r"C:\Users\tlhorng\Desktop\260123"
checkpoint_path = os.path.join(base_path, "mobilenet_imagenet100.pth")
labels_path = os.path.join(base_path, "Labels.json")

# 載入模型權重
state_dict = torch.load(checkpoint_path, map_location=device)
model = MobileNetV1(num_classes=100)
model.load_state_dict(state_dict)
model.to(device)
model.eval()

# 載入 Labels.json
with open(labels_path, "r") as f:
    labels_dict = json.load(f)

# class_names 對應 index 0~99
class_names = sorted(labels_dict.keys())[:100]
idx_to_label = [labels_dict[cid] for cid in class_names]

# ======================================================
# Translator
# ======================================================
translator = Translator()

# ======================================================
# Streamlit Web UI
# ======================================================
st.title("MobileNet ImageNet100 Classifier")

uploaded_file = st.file_uploader("Upload Image", type=["jpg", "jpeg", "png"])

if uploaded_file is not None:
    img = Image.open(uploaded_file).convert("RGB")
    st.image(img, caption="Input Image", width=400)

    x = transform(img).unsqueeze(0).to(device)

    with torch.no_grad():
        outputs = model(x)
        prob = torch.softmax(outputs, dim=1)
        top5 = torch.topk(prob, 5)

    # Top-5 標籤與機率
    labels = []
    scores = []
    for i in range(5):
        idx = top5.indices[0][i].item()
        score = top5.values[0][i].item() * 100  # 百分比
        if idx < len(idx_to_label):
            class_name = idx_to_label[idx]
        else:
            class_name = f"class{idx}"
        labels.append(class_name)
        scores.append(score)

    # 顯示文字，Top-1 紅字，其餘黑字
    st.write("### Top-5 Prediction")
    for i in range(5):
        # 翻譯英文到繁體中文
        try:
            zh_text = translator.translate(labels[i], src='en', dest='zh-tw').text
        except:
            zh_text = ""
        
        text = f"{i+1}. {labels[i]} ({zh_text}) : {scores[i]:.2f}%"
        
        if i == 0:
            st.markdown(f"<span style='color:red'>{text}</span>", unsafe_allow_html=True)
        else:
            st.write(text)

    # 長條圖（保留英文，不需翻譯）
    fig, ax = plt.subplots()
    colors = ["red" if i == 0 else "skyblue" for i in range(5)]
    ax.barh(labels[::-1], scores[::-1], color=colors[::-1])
    ax.set_xlabel("Probability (%)")
    ax.set_title("Top-5 Predictions")
    ax.set_xlim(0, 100)
    for i, v in enumerate(scores[::-1]):
        ax.text(v + 1, i, f"{v:.2f}%", va="center")
    st.pyplot(fig)